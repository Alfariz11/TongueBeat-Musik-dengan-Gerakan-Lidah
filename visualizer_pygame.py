...
class VisualizerPygame:
    ...
    def draw_arpeggiator(self, arp_data, hand_height, volume):
        ...
        pygame.draw.line(self.screen, (100, 255, 100), (10, y_hand), (self.width // 2 - 10, y_hand), 2)
    ...
    def draw_drum_visualization(self, drum_data):
        ...
        for i in range(16):
            ...
            if i == current_step:
                ...
                pygame.draw.circle(self.screen, color, (x + step_width // 2, y + 17), step_width // 2 + 3, 2)
            else:
            ...
            if is_beat:
                pygame.draw.circle(self.screen, (255, 200, 100), (x + step_width // 2, y - 6), 3)
        info_y = h - 55
    ...
    def draw_drum_velocity_bars(self, drum_data):
        ...
        for row, drum_name in enumerate(instrument_names):
            ...
            self._draw_text(drum_name.capitalize(), (start_x - 65, base_y - 6), instrument_colors[drum_name])
            for step in range(16):
                ...
                if velocity > 0:
                    ...
                    pygame.draw.rect(self.screen, color, (x, base_y - 2, step_width - 2, 2))
    ...
    def draw_camera_feed(self, frame, hand_data=None):
        ...
        if hand_data:
            for hand_label, data in hand_data.items():
                ...
                if hasattr(landmarks, "landmark"):
                    points = [(int(lm.x * target_width), int(lm.y * target_height)) for lm in landmarks.landmark]
                else:
                    points = [(int(x * target_width), int(y * target_height)) for x, y in landmarks]
                connections = [
                    (0, 1), (1, 2), (2, 3), (3, 4),
                    (0, 5), (5, 6), (6, 7), (7, 8),
